...
const rules = [
  ...
  },
  {
    ...
    validator: async (payload, tester) => {
      const internal = payload.result.aTags.filter((l) => l.href.includes(seoLint.host) || !l.href.includes('http'));
  ...
  },
  {
    ...
    validator: async (payload, tester) => {
      const external = payload.result.aTags.filter((l) => !l.href.includes(seoLint.host) && l.href.includes('http'));
