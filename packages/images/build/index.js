...
// src/index.ts
var import_worker_nodes = __toESM(require("worker-nodes"));
...
var config = {
  ...
  placeholder: {
    resize: {
      width: 10,
      fit: import_sharp.default.fit.cover
    },
    jpeg: {
      quality: 50,
      progressive: true,
...
var processImages = async ({
  ...
  try {
    ...
    const placeholderWorker = new import_worker_nodes.default(import_path.default.resolve(__dirname, "./workers/placeholder.js"), workerSettings);
    const originalS3Worker = new import_worker_nodes.default(import_path.default.resolve(__dirname, "./workers/saveOriginalToS3.js"), __spreadProps(__spreadValues({}, workerSettings), {
      autoStart: false
    }));
    const resizeWorker = new import_worker_nodes.default(import_path.default.resolve(__dirname, "./workers/resize.js"), workerSettings);
    ...
    for (const file of images) {
      if (!s3) {
        ...
        if (!import_fs_extra.default.existsSync(publicFolderDest)) {
          import_fs_extra.default.copyFileSync(file.src, publicFolderDest);
        }
      }
      if (!manifest[file.rel]) {
        const original = await (0, import_sharp.default)(file.src).toBuffer({ resolveWithObject: true });
        let s3Rel;
        if (s3) {
...
var plugin = {
  ...
  init: async (plugin2) => {
    ...
    if (typeof plugin2.config.imageManifest === "function") {
      ...
      const manifestLoc = import_path.default.join(plugin2.settings.rootDir, plugin2.config.imageManifest);
      if (import_fs_extra.default.existsSync(manifestLoc)) {
        try {
          const manifest = import_fs_extra.default.readFileSync(manifestLoc, { encoding: "utf-8" });
      ...
      run: async (payload) => {
        ...
        if (folders.length > 0) {
          ...
          plugin2.internal.imagesToProcess = folders.reduce((out, folder) => {
            import_fs_extra.default.ensureDirSync(import_path.default.join(settings.distDir, folder.output));
            const files = import_glob.default.sync(import_path.default.join(settings.rootDir, folder.src + `.{${imageFileTypes.join(",")}}`));
            if (Array.isArray(files)) {
              files.filter((file) => !file.split("/").pop().includes("-ejs")).forEach((file) => {
                ...
                const baseDir = import_path.default.join(settings.distDir, folder.output);
                import_fs_extra.default.ensureDirSync(baseDir);
                out.push({
                  src: file,
                  rel,
                  ext,
                  publicPrefix: import_path.default.join(baseDir, nameNoExt),
                  cachePrefix: config.cacheFolder && import_path.default.join(settings.rootDir, relNameNoExt.replace(folder.output, config.cacheFolder))
                });
              });
            }
            return out;
        ...
        if (plugin2.internal.imagesToProcess && Array.isArray(plugin2.internal.imagesToProcess) && plugin2.internal.imagesToProcess.length > 0) {
          ...
          if (!plugin2.debug && typeof plugin2.config.imageManifest === "string") {
            import_fs_extra.default.writeJsonSync(import_path.default.join(settings.rootDir, plugin2.config.imageManifest), plugin2.internal.manifest);
    ...
    },
    {
      ...
      run: async (payload) => {
        ...
        if (plugin2.config.addVanillaLazy) {
          const vanillaLazyNodeModules = import_path.default.join(settings.rootDir, "node_modules", "vanilla-lazyload", "dist", "lazyload.min.js");
          const vanillaLazyPublic = import_path.default.join(settings.distDir, plugin2.config.vanillaLazyLocation);
          if (!import_fs_extra.default.existsSync(vanillaLazyPublic)) {
            if (import_fs_extra.default.existsSync(vanillaLazyNodeModules)) {
              import_fs_extra.default.outputFileSync(vanillaLazyPublic, import_fs_extra.default.readFileSync(vanillaLazyNodeModules, { encoding: "utf-8" }));
            } else {
